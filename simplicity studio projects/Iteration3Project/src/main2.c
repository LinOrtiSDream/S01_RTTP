...
#define CHANNEL_SELECT_1 ChipSelect1_inv = 0, ChipSelect2_inv = 1
#define CHANNEL_SELECT_2 ChipSelect1_inv = 1, ChipSelect2_inv = 0
...
#define ADS1246_COMMAND_SYSOCAL 0x60
#define ADS1246_COMMAND_SYSGCAL 0x61
#define ADS1246_COMMAND_SELFOCAL 0x62
...
#define ADS1246_REGISTER_ID    0x0A // ID3 ID2 ID1 ID0 DRDY_MODE 0 0 0
...
void ads1246WriteAllRegisters(uint8_t regs[ADS1246_NUMBER_OF_REGISTERS])
{
	uint8_t i;
	regs[ADS1246_REGISTER_BCS] = (regs[ADS1246_REGISTER_BCS] & 0xC0) | 0x01;
	regs[ADS1246_REGISTER_BCS] = regs[ADS1246_REGISTER_VBIAS] & 0x03;
	regs[ADS1246_REGISTER_MUX1] = regs[ADS1246_REGISTER_MUX1] & 0x07;
	regs[ADS1246_REGISTER_SYS0] = regs[ADS1246_REGISTER_SYS0] & 0x7F;
	regs[ADS1246_REGISTER_ID] = regs[ADS1246_REGISTER_ID] & 0xF8;

	SPISendReceiveByte(ADS1246_COMMAND_WRITE_REGISTER  // команда Read 1-ый байт,
	...
	for (i=0; i < ADS1246_NUMBER_OF_REGISTERS; i++) {
		SPISendReceiveByte(regs[i]);
	}
}
...
enum calibrate_types {
	SYSTEM_OFFSET_CALIBRATION,
	SYSTEM_GAIN_CALIBRATION,
	SELF_OFFSET_CALIBRATION,
	SYSTEM_OFFSET_CALIBRATION_USING_MUX,
	SYSTEM_GAIN_CALIBRATION_USING_MUX,
};

void ads1246Calibrate(enum calibrate_types calibrate_type) {
	uint8_t store_mux;

	SPISendReceiveByte(ADS1246_COMMAND_SWITCH_TO_SDATAC);
	while (Data_RDY1_inv) {};

	if (calibrate_type == SYSTEM_OFFSET_CALIBRATION_USING_MUX
		|| calibrate_type == SYSTEM_GAIN_CALIBRATION_USING_MUX) {
			store_mux = ads1246ReadRegister(ADS1246_REGISTER_MUX1);
		}
	switch (calibrate_type) {
		case SYSTEM_OFFSET_CALIBRATION:
			SPISendReceiveByte(ADS1246_COMMAND_SYSOCAL);
			break;
		case SYSTEM_GAIN_CALIBRATION:
			SPISendReceiveByte(ADS1246_COMMAND_SYSGCAL);
			break;
		case SELF_OFFSET_CALIBRATION:
			SPISendReceiveByte(ADS1246_COMMAND_SELFOCAL);
			break;
		case SYSTEM_OFFSET_CALIBRATION_USING_MUX:
			// MUX = 001: Offset calibration. The analog inputs are disconnected and
			// AINP and AINN are internally connected to mid-supply (AVDD + AVSS) / 2.
			ads1246WriteRegister(ADS1246_REGISTER_MUX1, 0x01);
			SPISendReceiveByte(ADS1246_COMMAND_SYSOCAL);
			break;
		case SYSTEM_GAIN_CALIBRATION_USING_MUX:
			// MUX = 010: Gain calibration. The analog inputs are connected to the
			// voltage reference.
			ads1246WriteRegister(ADS1246_REGISTER_MUX1, 0x02);
			SPISendReceiveByte(ADS1246_COMMAND_SYSGCAL);
			break;
	}
	while (Data_RDY1_inv) {};
	...
	if (calibrate_type == SYSTEM_OFFSET_CALIBRATION_USING_MUX
		|| calibrate_type == SYSTEM_GAIN_CALIBRATION_USING_MUX) {
			ads1246WriteRegister(ADS1246_REGISTER_MUX1, store_mux);
		}
...
enum DATAAGE {
   ...
   struct exchangeStruct {
	   ...
	   union {
		   char trash[4];
		   uint8_t reg_number;
		   enum calibrate_types calibrate_type;
	   } cmd;
	   int32_t dataReaded1;
	   int32_t dataReaded2;
   enum DATAAGE dataAge; // !!!DANGER 'enum' can be longer than byte / depends on compiler
	   uint8_t adc_regs[ADS1246_NUMBER_OF_REGISTERS];

   } xdata dataSet = {sizeof(struct exchangeStruct), 3, INIT_SYSTEM, "---", 0, DATA_NO_DATA};

   struct exchangeStruct xdata cmdSet;
...
//	   writeDataToHost();
	   ...
	   workstate = READ_REGISTERS;
   }
   if (INTVAL & USBX_TX_COMPLETE)                          // USB Write complete
   {
	   if (dataSet.dataAge == DATA_SENDING) {dataSet.dataAge = DATA_OLD;}
   }
   if (INTVAL & USBX_RX_COMPLETE)                          // USB Read complete
   {
...
void main(void)
{
   ...
   USBX_apiCallbackEnable(myAPICallback);
   ...
   while (1) {
	   switch (workstate) {
	   	   case INIT_SYSTEM:
	   		   	CHANNEL_SELECT_1;			// выбор микросхемы 1
				ads1246Reset();
				// работаем с Мсх 2
				// Посылаем в АЦП команду SDATAC - прекращение непрерывного считывания данных
				// для считывания значения регистра
				...
				CHANNEL_SELECT_2;			// выбор микросхемы 2
				ads1246Reset();
				// работаем с Мсх 2
				// Посылаем в АЦП команду SDATAC - прекращение непрерывного считывания данных
				// для считывания значения регистра
				...
				while (Data_RDY2_inv) {}; // ждем появления Data_RDY2 = 0
	   	   ...
	   	   case DEFAULT_READ:
			   ...
			   while ( workstate == DEFAULT_READ ) // continuously read data
			   {
					...
					dataReaded *= 256;
					dataReaded += SPISendReceiveByte(COMMAND_NOP);
					dataSet.dataReaded1 = dataReaded;
					...
					dataReaded *= 256;
					dataReaded += SPISendReceiveByte(COMMAND_NOP);
					dataSet.dataReaded2 = dataReaded;
					dataSet.dataAge = (dataSet.dataAge == DATA_OLD ? DATA_NEW : DATA_LOST);
					//TMR2RL= dataSet.dataReaded[0] * 256 + dataSet.dataReaded[1];  ;
					dataSet.workstate = DEFAULT_READ;
					writeDataToHost();
				};
			   break;
	   	   case READ_REGISTERS:
			   SPISendReceiveByte(ADS1246_COMMAND_SWITCH_TO_SDATAC);
	   		   while (Data_RDY1_inv) {};
	   		   ads1246ReadAllRegisters(dataSet.adc_regs);
	   		   writeDataToHost();
	   		   workstate = DEFAULT_READ;
	   		   break;
	   	   case WRITE_ONE_REGISTER:
			   SPISendReceiveByte(ADS1246_COMMAND_SWITCH_TO_SDATAC);
	   		   while (Data_RDY1_inv) {};
	   		   ads1246WriteRegister(cmdSet.cmd.reg_number, cmdSet.adc_regs[cmdSet.cmd.reg_number]);
	   		   workstate = READ_REGISTERS;
	   		   break;
	   	   case CALIBRATE:
			   SPISendReceiveByte(ADS1246_COMMAND_SWITCH_TO_SDATAC);
	   		   while (Data_RDY1_inv) {};
	   		   ads1246Calibrate(cmdSet.cmd.calibrate_type);
	   		   workstate = READ_REGISTERS;
	   		   break;
/*
	   	   case SET_SPEED:
